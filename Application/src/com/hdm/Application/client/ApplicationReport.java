...
public class ApplicationReport implements EntryPoint {
	...
	//Buttons der Navigation. M√ºssen final sein, damit Callbacks sie ver√§ndern k√∂nnen
	...
	final Button showEditor = new Button("Zur¸ck zum Editor");
...
//		
		...
		welcomeLabel.setText("Willkommen im Report Generator, bitte w‰hlen Sie in der Navigation zuerst aus welchen Report Sie generieren mˆchten.");
		...
		/*
		 * Login Status mittels LoginService checken. √úbergabeparameter isReportGen ist true,
...
protected void loadRadiobuttonPanel() {
	...
	final RadioButton dueDateButton = new RadioButton("Radiobutton-Group","F‰lligkeitsdatum");
	...
	dueDateButton.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
		...
		//ClickHandler f¸r den Button zum Report nicht besuchter AppUsere
		showReportAllUserNotesButton.addClickHandler(new ClickHandler() {
			@Override
			...
			public void onClick(ClickEvent event) {
				...
				/*
				 * Pr√ºfung ob zum eingeloggten AppUser ein AppUser in der Datenbank vorhanden ist.
		...
		//ClickHandler f√ºr den Button zum Report nach Suchprofil
		
		
		//ClickHandler f√ºr den Button zum Navigieren zur√ºck zum Editor
...
//		     loginTextPanel.add(passwordBox);
	...
	/**
	 * Diese Nested Class wird als Callback f√ºr das Erzeugen des
	 * AllAppUseresNotVisitedReport ben√∂tigt.
	 * 
	 * @author bueschel
	 */
	class createAllNotesFromUserReportCallback implements
		...
		public void onSuccess(AllFilteredNotes report) {
			if (report != null) {
				//Um den Report in HTML-Text zu √ºberf√ºhren benˆtigen wir einen HTMLReportWriter
				HTMLReportWriter writer = new HTMLReportWriter();
				//Wir √ºbergeben den erhaltenen Report an den HTMLReportWriter
				writer.process(report);
				//Wir leeren das DetailPanel, damit etwaige vorherige Reports nicht mehr angezeigt werden
				detailPanel.clear();
				//Wir bef√ºllen das DetailPanel mit dem HTML-Text den wir vom ReporWriter erhalten
