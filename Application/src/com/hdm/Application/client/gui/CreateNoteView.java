...
public class CreateNoteView extends Update {
	...
	// private String currentDateTime = new
	...
	protected void run() {
		...
		savePermissionButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				savePermissionButton
						.setStylePrimaryName("savePermission-button");
				...
				adminService.searchUserByGoogleID(googleID,
						searchUserByGoogleIDCallback());
				...
				if (user != null) {
					...
					if (readButton.getValue() == false
							&& editButton.getValue() == false) {
						Window.alert("Bitte waehlen Sie eine Art der Berechtigung aus");
					}

					adminService.createPermission(permission,
		...
		createButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				 ...
				 * Mehrfach-Klicks und daraus entstehende, unnoetige Eintraege
				 * in der Datenbank zu verhindern.
				 */
	...
	private AsyncCallback<Void> createPermissionCallback() {
		AsyncCallback<Void> asyncCallback = new AsyncCallback<Void>() {
			...
			public void onSuccess(Void result) {
				ClientsideSettings.getLogger().severe(
						"Success CreatePermissionCallback: "
								+ result.getClass().getSimpleName());
			}
		};
		return asyncCallback;
	...
	private AsyncCallback<AppUser> searchUserByGoogleIDCallback() {
		AsyncCallback<AppUser> asyncCallback = new AsyncCallback<AppUser>() {
			...
			public void onSuccess(AppUser result) {
				ClientsideSettings.getLogger().severe(
						"Success SearchUserByGoogleIDCallback: "
								+ result.getClass().getSimpleName());
				user = result;
			}
		};
