 ...
 * Diese Klasse stellt Methoden zur Verfuegung, die das erstellen, editieren,
 * auslesen/suchen und loeschen der gewuenschten Datensaezte erlauben. Die
 * Mapperklasse stellt die Verbindungsschicht zwischen Datenbank und
 * Applikationslogik dar. Datenbank-Strukturen koennen in Objekte umgewandelt
 * werden, jedoch auch Objekte in Datenbankstrukturen
 */
...
public class NoteMapper {
	 ...
	 * es kann nur eine Instanz von NoteMapper existieren.
	 * 
	...
	/**
	 * Ein Note-Objekt wird in der Datenbank hinzugefuegt. Der Primaerschluessel
	 * wird ueberprueft und gegebenenfalls neu zugeordnet.
	 * 
	 * @param note
	...
	public Note createNote(Note note) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT MAX(nID) AS maxnID"
					+ " FROM notes");

			String modDate = null;
			if (note.getnModDate() != null) {
				SimpleDateFormat mySQLformat = new SimpleDateFormat(
						"yyyy-MM-dd");
		...
		/**
		 * Note wird zurueckgegeben, da sich das Objekt eventuell im Laufe der
		 * Methode veraendert hat
		 */
	...
	public Note updateNote(Note note) {
		...
		try {
			Statement stmt = con.createStatement();
			stmt.executeUpdate("UPDATE notes " + "SET nTitle=\""
					+ note.getnTitle() + "\"," + "nSubtitle=\""
					+ note.getnSubtitle() + "\"" + "nContent=\""
					+ note.getnContent() + "\"," + "source=\""
					+ note.getSource() + "\"," + "nCreDate=\""
					+ note.getnCreDate() + "nModDate=\"" + note.getnModDate()
					+ "userID=\"" + note.getUserID() + "WHERE nID="
					+ note.getnID());
			// modification Date hinzufuegen --> wie mach ich dass es auf
			// jednefall automatisch hinzugefuegt wird?
	...
	public static void deleteAllNotebookNotes(Notebook nb) {
		...
		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT nbid, nid, FROM permissions"
			...
			while (rs.next()) {
				...
				stmt2.executeUpdate("DELETE FROM notes" + "WHERE nID="
						+ idInt.intValue());
			}
	...
	/**
	 * Eine spezielle Notiz (Note) wird ueber die NoteID (nbID) gesucht. Es wird
	 * genau ein Objekt zurueck gegeben
	 * 
	 * @param nID
	 *            (Primaerschluessel DB)
	 * @return Note-Objekt, das die gesuchte nID besitzt - null bei nicht
	 *         vorhandenem Tupel
	 */
	...
	public Note findByID(int nID) {
		...
		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt
					.executeQuery("SELECT nID, nbID, userID, nTitle, nSubtitle, nContent, source, nCreDate, nModDate"
	 ...
	 * 
	 * @return Vektor mit Note Objekten, der alle Notes enthaelt. Trifft eine
	 *         Exception ein wird ein teilweise gefuellter oder leerer Vektor
	 *         ausgegeben
	 */
	 ...
	 * 
	 * @param nTitle
	 * @return Vektor mit Note-Objekten, die den gesuchten Note-Titel (nTitle)
	...
	public Vector<Note> findByNotebook(Notebook notebook) {
		...
		try {
			...
			// Statement ausfuellen und als Query an DB schicken
			ResultSet rs = stmt
					.executeQuery("SELECT nid, nbid, title, subtitle, content, source, creadate, moddate FROM notes"
